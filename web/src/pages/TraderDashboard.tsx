...
function TraderDashboardContent() {
  ...
  const selectedTrader = traders?.find(
    (t) => t.trader_id === effectiveSelectedId
  )
  ...
  const chartData = useMemo(() => {
    ...
    return filterByRange(mapped, timeRange) as Array<{
      timestamp: string
      value: number
    }>
  ...
  const { todayPnl, todayPnlPct } = useMemo(() => {
    if (!equityHistory || equityHistory.length < 2)
      return { todayPnl: 0, todayPnlPct: 0 }
  ...
  if (tradersError) {
    return (
      <ErrorState
        ...
        description="We couldn't load your trader data. Please try again."
  ...
  // ── Loading skeleton (waiting for selected trader) ───────────────────────────────
  if (!selectedTrader) {
    return <DashboardSkeleton />
  }
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-base font-bold text-white">
                  {positions?.length ?? account?.position_count ?? 0}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <p
                ...
                style={{ color: todayPnl >= 0 ? GREEN : RED }}
              >
