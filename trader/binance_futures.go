...
func NewFuturesTrader(apiKey, secretKey string, userId string) *FuturesTrader {
	...
	if err := trader.setDualSidePosition(); err != nil {
		log.Warn().Msgf("⚠️ 设置双向持仓模式失败: %v (如果已是双向模式则忽略此警告)", err)
	}
...
func (t *FuturesTrader) setDualSidePosition() error {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "No need to change position side") {
			log.Info().Msgf("  ✓ 账户已是双向持仓模式（Hedge Mode）")
			return nil
		}
	...
	log.Info().Msgf("  ✓ 账户已切换为双向持仓模式（Hedge Mode）")
	log.Info().Msgf("  ℹ️  双向持仓模式允许同时持有多单和空单")
	return nil
}
...
func syncBinanceServerTime(client *futures.Client) {
	...
	if err != nil {
		log.Warn().Msgf("⚠️ 同步币安服务器时间失败: %v", err)
		return
	}
	...
	log.Info().Msgf("⏱ 已同步币安服务器时间，偏移 %dms", offset)
}
...
func (t *FuturesTrader) GetBalance() (map[string]interface{}, error) {
	...
	if t.cachedBalance != nil && time.Since(t.balanceCacheTime) < t.cacheDuration {
		...
		log.Info().Msgf("✓ 使用缓存的账户余额（缓存时间: %.1f秒前）", cacheAge.Seconds())
		return t.cachedBalance, nil
	}
	t.balanceCacheMutex.RUnlock()

	// 缓存过期或不存在，调用API
	log.Info().Msgf("🔄 缓存过期，正在调用币安API获取账户余额...")
	account, err := t.client.NewGetAccountService().Do(context.Background())
	if err != nil {
		log.Error().Msgf("❌ 币安API调用失败: %v", err)
...
func (t *FuturesTrader) GetPositions() ([]map[string]interface{}, error) {
	...
	if t.cachedPositions != nil && time.Since(t.positionsCacheTime) < t.cacheDuration {
		...
		log.Info().Msgf("✓ 使用缓存的持仓信息（缓存时间: %.1f秒前）", cacheAge.Seconds())
		return t.cachedPositions, nil
	}
	t.positionsCacheMutex.RUnlock()

	// 缓存过期或不存在，调用API
	log.Info().Msgf("🔄 缓存过期，正在调用币安API获取持仓信息...")
...
func (t *FuturesTrader) SetMarginMode(symbol string, isCrossMargin bool) error {
	...
	if err != nil {
		...
		if contains(err.Error(), "Multi-Assets mode") || contains(err.Error(), "-4168") || contains(err.Error(), "4168") {
			...
			log.Info().Msgf("  💡 提示：如需使用逐仓模式，请在币安关闭多资产模式")
			return nil
		}
	...
	log.Info().Msgf("  ✓ %s 仓位模式已设置为 %s", symbol, marginModeStr)
	return nil
}
...
func (t *FuturesTrader) SetLeverage(symbol string, leverage int) error {
	...
	if currentLeverage == leverage && currentLeverage > 0 {
		log.Info().Msgf("  ✓ %s 杠杆已是 %dx，无需切换", symbol, leverage)
		return nil
	}
	...
	if err != nil {
		...
		if contains(err.Error(), "No need to change") {
			log.Info().Msgf("  ✓ %s 杠杆已是 %dx", symbol, leverage)
			return nil
		}
		return fmt.Errorf("设置杠杆失败: %w", err)
	}

	log.Info().Msgf("  ✓ %s 杠杆已切换为 %dx", symbol, leverage)

	// 切换杠杆后等待5秒（避免冷却期错误）
	log.Info().Msgf("  ⏱ 等待5秒冷却期...")
...
func (t *FuturesTrader) OpenLong(symbol string, quantity float64, leverage int) (map[string]interface{}, error) {
	...
	if err := t.CancelAllOrders(symbol); err != nil {
		log.Warn().Msgf("  ⚠ 取消旧委托单失败（可能没有委托单）: %v", err)
	}
	...
	log.Info().Msgf("✓ 开多仓成功: %s 数量: %s", symbol, quantityStr)
	log.Info().Msgf("  订单ID: %d", order.OrderID)
...
func (t *FuturesTrader) OpenShort(symbol string, quantity float64, leverage int) (map[string]interface{}, error) {
	...
	log.Info().Msgf("✓ 开空仓成功: %s 数量: %s", symbol, quantityStr)
	log.Info().Msgf("  订单ID: %d", order.OrderID)
...
func (t *FuturesTrader) CloseLong(symbol string, quantity float64) (map[string]interface{}, error) {
	...
	log.Info().Msgf("✓ 平多仓成功: %s 数量: %s", symbol, quantityStr)
...
func (t *FuturesTrader) CloseShort(symbol string, quantity float64) (map[string]interface{}, error) {
	...
	log.Info().Msgf("✓ 平空仓成功: %s 数量: %s", symbol, quantityStr)
...
func (t *FuturesTrader) CancelStopLossOrders(symbol string) error {
	...
	for _, order := range orders {
		...
		if orderType == futures.OrderTypeStopMarket || orderType == futures.OrderTypeStop {
			...
			canceledCount++
			log.Info().Msgf("  ✓ 已取消止损单 (订单ID: %d, 类型: %s, 方向: %s)", order.OrderID, orderType, order.PositionSide)
		}
	}
...
func (t *FuturesTrader) CancelTakeProfitOrders(symbol string) error {
	...
	for _, order := range orders {
		...
		if orderType == futures.OrderTypeTakeProfitMarket || orderType == futures.OrderTypeTakeProfit {
			...
			canceledCount++
			log.Info().Msgf("  ✓ 已取消止盈单 (订单ID: %d, 类型: %s, 方向: %s)", order.OrderID, orderType, order.PositionSide)
		}
	}
...
func (t *FuturesTrader) CancelAllOrders(symbol string) error {
	...
	log.Info().Msgf("  ✓ 已取消 %s 的所有挂单", symbol)
	return nil
}
...
func (t *FuturesTrader) CancelStopOrders(symbol string) error {
	...
	for _, order := range orders {
		...
		if orderType == futures.OrderTypeStopMarket ||
			...
			if err != nil {
				log.Warn().Msgf("  ⚠ 取消订单 %d 失败: %v", order.OrderID, err)
				continue
			}
...
func (t *FuturesTrader) SetStopLoss(symbol string, positionSide string, quantity, stopPrice float64) error {
	...
	log.Info().Msgf("  止损价设置: %.4f", stopPrice)
	return nil
}
...
func (t *FuturesTrader) SetTakeProfit(symbol string, positionSide string, quantity, takeProfitPrice float64) error {
	...
	log.Info().Msgf("  止盈价设置: %.4f", takeProfitPrice)
	return nil
}
...
func (t *FuturesTrader) GetSymbolPrecision(symbol string) (int, error) {
		...
		if s.Symbol == symbol {
			...
			for _, filter := range s.Filters {
				if filter["filterType"] == "LOT_SIZE" {
					...
					log.Info().Msgf("  %s 数量精度: %d (stepSize: %s)", symbol, precision, stepSize)
	...
	log.Warn().Msgf("  ⚠ %s 未找到精度信息，使用默认精度3", symbol)
	return 3, nil // 默认精度为3
}
